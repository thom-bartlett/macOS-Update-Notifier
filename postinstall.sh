...
launch_agent_plist_name='com.macupdate.plist'
...
if [[ $3 == "/" ]] ; then
  ...
  if [ ! -e "$3/${launch_agent_base_path}${launch_agent_plist_name}" ]; then
    ...
    /bin/launchctl unload "$3${launch_agent_base_path}${launch_agent_plist_name}"
    # Load the launch agent
    /bin/launchctl load -w "$3${launch_agent_base_path}${launch_agent_plist_name}"
fi
